...
//        emailService.enviarEmailComTemplateDelete(pessoaDTO);
    ...
    public void mandarEmailPessoasSemEndereco() {
        List<PessoaEntity> lista = pessoaRepository.buscarPessoasComEnderecoNull();
        lista.forEach(pessoaEntity -> {
            try {
                emailService.enviarEmailComTemplateSchedule(pessoaEntity);
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    ...
    @Scheduled(cron = "0 0 8 23 12 *")
    public void mandarEmailNatal() {
        List<PessoaEntity> lista = pessoaRepository.findAll();
        lista.forEach(pessoaEntity -> {
            try {
                emailService.enviarEmailComTemplateScheduleMarketing(pessoaEntity);
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    ...
    @Scheduled(cron = "0 0 8,20 * * *")
    public void mandarEmailPessoasSemEnderecoKafka() {
        List<PessoaEntity> lista = pessoaRepository.buscarPessoasComEnderecoNull();
        lista.forEach(pessoaEntity -> {
            try {
                producer.sendMessageDTO(emailService.enviarEmailComTemplateScheduleKafka(pessoaEntity));
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
    ...
    @Scheduled(cron = "0 0 8 23 12 *")
    public void mandarEmailNatalKafka() {
        List<PessoaEntity> lista = pessoaRepository.findAll();
        lista.forEach(pessoaEntity -> {
            try {
                producer.sendMessageDTO(emailService.enviarEmailComTemplateScheduleMarketingKafka(pessoaEntity));
            } catch (Exception e) {
                e.printStackTrace();
            }
        });
